...
TEAM_NEUTRAL = 300
TEAM_MAX = 400
MINION_SPAWN_START_TIME = 65
START_OF_GAME_MONSTER_SPAWN_DELAY = 22
RED_BLUE_OFFSET = 2
CAMP_TIME_OFFSET = 1
START_OF_GAME_ADDITIONAL_DELAY_CRAB = 109
DRAGON_SPAWN_TIME = 300 - MINION_SPAWN_START_TIME
HERALD_SPAWN_TIME = 600 - MINION_SPAWN_START_TIME
BARON_SPAWN_TIME = 1200 - MINION_SPAWN_START_TIME
ELEMENTAL_DRAGON_SPAWN_RATE = 300
ELDER_DRAGON_SPAWN_TIME = 2100
ELDER_DRAGON_SPAWN_RATE = 360
HOSTILE = 1
INACTIVE = 0
CRAB_RANDOM_CHANCE = 0
CRAB_FIRST_DRAGON = 1
CRAB_FIRST_BARON = 1
CRAB_RANDOM_READY = 0
CRAB_TOP_STREAK = 0
CRAB_BOTTOM_STREAK = 0
CRAB_NEW_LOCATION = 0
...
function NeutralMinionInit()
  ...
  for L3_3 = 1, NUMBER_OF_CAMPS do
    ...
    L4_4[NeutralMinionCamps[L3_3].Name] = NeutralMinionCamps[L3_3].GroupDelaySpawnTime + MINION_SPAWN_START_TIME
...
function NeutralMinionDeath(A0_31, A1_32, A2_33, A3_34)
  ...
  if GetUnitSkinName(A1_32) == "Sru_Crab" then
    if CRAB_FIRST_BARON == 1 then
      CRAB_FIRST_BARON = 0
    elseif CRAB_FIRST_DRAGON == 1 then
      CRAB_FIRST_DRAGON = 0
      CRAB_RANDOM_READY = 1
      SetWorldVar("CRAB_RANDOM_READY", CRAB_RANDOM_READY)
    end
    if CRAB_BOTTOM_STREAK >= 4 then
    elseif 4 <= CRAB_TOP_STREAK then
    else
    end
    CRAB_RANDOM_CHANCE = GetRandomIntInRangeLua(1, 100)
    if 50 >= CRAB_RANDOM_CHANCE then
      CRAB_NEW_LOCATION = 15
      CRAB_BOTTOM_STREAK = CRAB_BOTTOM_STREAK + 1
      CRAB_TOP_STREAK = 0
    else
      CRAB_NEW_LOCATION = 16
      CRAB_TOP_STREAK = CRAB_TOP_STREAK + 1
      CRAB_BOTTOM_STREAK = 0
    end
    SetWorldVar("CRAB_NEW_LOCATION", CRAB_NEW_LOCATION)
  end
  for _FORV_10_ = 1, #NeutralMinionCamps do
    for _FORV_14_ = 1, #NeutralMinionCamps[_FORV_10_].UniqueNames do
      for _FORV_18_ = 1, #NeutralMinionCamps[_FORV_10_].UniqueNames[_FORV_14_] do
        if GetUnitSkinName(A1_32) == "Sru_Crab" then
          if CRAB_RANDOM_READY == 1 then
            if CRAB_NEW_LOCATION == 15 then
              NeutralMinionCamps[15].GroupsChance = {100}
              NeutralMinionCamps[16].GroupsChance = {0}
              L5_36 = 15
              NeutralMinionCamps[15].AliveTracker[_FORV_18_] = false
            else
              NeutralMinionCamps[15].GroupsChance = {0}
              NeutralMinionCamps[16].GroupsChance = {100}
              L5_36 = 16
              NeutralMinionCamps[16].AliveTracker[_FORV_18_] = false
            end
          end
        elseif NeutralMinionCamps[_FORV_10_].UniqueNames[_FORV_14_][_FORV_18_] == A0_31 then
          L5_36 = _FORV_10_
          NeutralMinionCamps[_FORV_10_].AliveTracker[_FORV_18_] = false
        end
      end
    end
  end
  if L5_36 > 0 then
    for _FORV_10_ = 1, #NeutralMinionCamps[L5_36].AliveTracker do
      if NeutralMinionCamps[L5_36].AliveTracker[_FORV_10_] == true then
        L4_35 = false
      end
    end
    if L4_35 == true then
